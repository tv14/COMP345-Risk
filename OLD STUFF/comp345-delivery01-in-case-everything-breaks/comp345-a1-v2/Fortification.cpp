...
void Fortification::setMovingFrom(){
    ...
    while (check)
    {
		...
		string chosenmovingcountry = "";
        getline(std::cin, chosenmovingcountry);
...
void Fortification::setMovingTo(){
   std::string chosenmovingto;
    ...
    while (check) {
		...
		string chosenmovingto = "";
        getline(std::cin, chosenmovingto);
...
bool Fortification::hasAPath(Country &c){
	...
	const int temp = this->getNumberOfCountries();
	bool *checked;
	checked = new bool[temp];//creates a boolean which checks if the path has already visited this country
