...
void driver::run() {
	...
	for (vector<Player>::iterator it = tempplayerlist.begin(); it != tempplayerlist.end(); ++it) {
		cout << it->getPlayerName() << endl;
	}

	/* USE THE FOLLOWING FOR DELIVERY 1 */
	// these are the actual players for the match. the tempplayerlist used above is just for demo purposes,
	// to show that the startup phase exists and what happens during it
	Player p1("A"), p2("B");
	...
	// map loading
	/* placeholder code from assignment 1 */
	string filename = "";
	...
	/*
	for (int i = 0; i < countriescount; i++) // countriescount does not currently exist: need map implementation
	{
	...
	*/
	...
	/* HARDCODED loop to iterate through exactly FIVE elements in countries array and display their respective owners */
	for (int i = 0; i < 5; ++i) {
		cout << countries[i].getName() << "'s owner is " << countries[i].getOwner()->getPlayerName() << endl;
	}
