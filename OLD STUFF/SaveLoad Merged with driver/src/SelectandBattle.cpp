...
bool SelectandBattle::noEnemy() {
   ...
   {
	   string s = listofcountries[i]->getName();
       if (attackingcountry->isAdjacent(s))
       {
...
void SelectandBattle::selectDefendingCountry() {
    ...
    while (check) {
        ...
        for (int i=0;i<this->NUMBER_OF_COUNTRIES;i++)
        {
            ...
            {
                string s = listofcountries[i]->getName();
                if (this->attackingcountry->isAdjacent(s) && attackingcountry->getOwner()->getPlayerName()!=listofcountries[i]->getOwner()->getPlayerName())//checks to see if the inputted country is an adjacent enemy country
                {
