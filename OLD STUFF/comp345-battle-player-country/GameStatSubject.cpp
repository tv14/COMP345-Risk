...
void GameStatSubject::deriveNumberOfCountriesOwned(){
    ...
    for (int indexofplayers=0;indexofplayers<this->listofplayers.size();indexofplayers++)//calculates for all players
    {
        int numberofcountriesowned=0;
        for (int indexofcountries=0;indexofcountries<this->listofcountries.size();indexofcountries++)//checks each country for every player
        {
            if (this->listofcountries[indexofcountries].getOwner()->getPlayerName()==listofplayers[indexofplayers].getPlayerName())//if the player owns the country, the country count increments
            {
...
void GameStatSubject::deriveTotalNumberofArmiesOwned(){
    ...
    for (int indexofplayers=0;indexofplayers<this->listofplayers.size();indexofplayers++)//calculates for all players
    {
        ...
        for (int indexofcountries=0;indexofcountries<this->listofcountries.size();indexofcountries++)//checks each country for every player
        {
            if (this->listofcountries[indexofcountries].getOwner()->getPlayerName()==listofplayers[indexofplayers].getPlayerName())//if the player owns the country, the army count is added to the total army count
            {
                numberofarmiesowned+=this->listofcountries[indexofcountries].getArmyCount();
            }
        }
...
void GameStatSubject::deriveTotalNumberOfCardsOwned(){
    ...
    for (int indexofplayers=0;indexofplayers<this->listofplayers.size();indexofplayers++)//calculates for each player
    {
        this->listofnumberofcardsowned.push_back(this->listofplayers[indexofplayers].getNumberOfCards());//pushes the number of cards the player has into the list
    }
}
