...
void WorldControlledDecorator::Update(){
    ...
    for (int indexofplayers=0;indexofplayers<listofplayers->size();indexofplayers++)//for each player
    {
		...
		string numbertostring = to_string(worldpercentageowned);
...
void WorldControlledDecorator::update(std::string str, std::string playername, std::string phasename){};

void WorldControlledDecorator::printself(){};