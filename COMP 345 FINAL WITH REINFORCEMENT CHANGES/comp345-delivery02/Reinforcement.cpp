...
void Reinforcement::calculateReinforcement(){
    ...
    for (int indexofcountries=0;indexofcountries<listofcountries.size();indexofcountries++)//checks ever
    {
        ...
        if (countryC.getOwner()->getPlayerName()==currentplayer->getPlayerName())
        {
     ...
     for (int indexofcontinents=0;indexofcontinents<listofcontinents.size();indexofcontinents++)//for all continent
     {
         ...
         for (int indexofcountriesincontinent=0;indexofcountriesincontinent<listofcontinents[indexofcontinents].getNumCountries();indexofcountriesincontinent++)//looks at each continent's countries
         {
             ...
             {
               	Country countryC = listofcountries[indexofcountries];
                 if (countryC.getName()==countryincontinent && countryC.getOwner()->getPlayerName()==currentplayer->getPlayerName())//checks to see if the player owns the country
                 {
                     check++;
                 }
             }
...
bool Reinforcement::calculateCardsReinforcement(){
    ...
    if (Reinforcement::useCards())
    {
        ...
        while (check)
        {
			if (currentplayer->getNumberOfCards() >= 6)//if a player has 6 or more cards, he must spend some to get reinforcements
			{
				cout << "You have 6 or more cards and must spend some to obtain reinforcements" << std::endl;
				userinput = "y";
			}
			else {//otherwise he can be given the choice 
                ...
                while(secondcheck)
                {
                    ...
                    if (userinput=="1")
                    {
                        if (currentplayer->getSoldierCards()>=3)//checks to see if the player has enough soldier cards
                        {
							...
							this->currentplayer->setCurrentReinforcements(this->reinforcements);
...
void Reinforcement::chooseCountry(){
    ...
    while (check)
    {
        ...
        {
        	Country countryC = listofcountries[indexofcountries];
            if (countryC.getName()==chosencountry)//program checks if user inputted a proper country
            {
